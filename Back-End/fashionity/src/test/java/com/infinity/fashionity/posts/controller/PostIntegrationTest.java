...
//                post.getComments().addAll(commentEntities);
    ...
    public class PostsGetAllTest{
        ...
        public void successWithDefaultGetWithoutAuthorization() throws Exception {
            ...
            List<PostListDTO.Post> expected = postLists.stream()
                    .sorted((o1, o2) -> {
                        ...
                        if(l1 == l2){
                            return Long.compare(o2.getSeq(),o1.getSeq());
                        }
            ...
            for(int i=0;i<response.getPosts().size();i++){
                ...
                PostListDTO.Post expect = expected.get(i);
        ...
        @Test
        @DisplayName("success with default get with authorization")
        public void successWithDefaultGetWithAuthorization() throws Exception {
            String token = memberLogin(memberSeq).getAccessToken();
            MvcResult result = sendRequest(token, dto)
                    .andExpect(status().isOk())
                    .andReturn();
            ...
            List<PostListDTO.Post> expected = postLists.stream()
                    ...
                    .map(postEntity -> PostListDTO.Post.builder()
                            ...
                            .profileImg(postEntity.getMember().getProfileUrl())
                            .commentCount(comments.get(postEntity.getSeq()).size())
                            .liked(postLikes.get(postEntity.getSeq()).stream().filter(postLikeEntity -> postLikeEntity.getMember().getSeq() == memberSeq).findAny().isPresent())
                            .content(postEntity.getContent())
                            .images(images.get(postEntity.getSeq()).stream().map(e -> e.getUrl()).collect(Collectors.toList()))
                            .name(postEntity.getMember().getNickname())
                            .build())
